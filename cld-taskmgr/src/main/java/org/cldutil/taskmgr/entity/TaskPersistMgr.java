package org.cldutil.taskmgr.entity;
...
import org.cldutil.datastore.DBException;
import org.cldutil.datastore.QueryUtil;
...
public class TaskPersistMgr {
	...
	public static Task getTask(SessionFactory sf, String tid){
		Session session = sf.openSession();
		try{
			Task t = (Task) session.get("org.cldutil.taskmgr.entity.Task", tid);
			return t;
		}finally{
			session.close();
		}
	...
	public static boolean removeTasksById(SessionFactory sessionFactory, Collection<String> tkl){
		...
		try {
			...
			while (tklit.hasNext()){
				...
				Task t = (Task) session.get("org.cldutil.taskmgr.entity.Task", id);
				if (t!= null){
					session.delete(t);
				}else{
